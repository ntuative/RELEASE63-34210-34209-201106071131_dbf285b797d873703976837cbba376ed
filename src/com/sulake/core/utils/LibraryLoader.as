package com.sulake.core.utils
{
   ...
   public class LibraryLoader extends EventDispatcher implements IDisposable
   {
      ...
      protected static const const_1985:uint = 1;
      
      protected static const const_1987:uint = 2;
      
      protected static const const_1988:uint = 4;
      
      protected static const const_1986:uint = 3;
      ...
      public static const const_1306:Boolean = false;
      
      public static const const_1782:int = 2;
      
      private static var var_986:Array = new Array();
      
      private static var var_807:Array = new Array();
      ...
      protected var var_253:URLRequest;
      
      protected var _manifest:XML;
      
      protected var var_703:Class;
      
      protected var var_493:uint = 0;
      
      protected var _name:String;
      
      protected var var_1119:Boolean = false;
      
      protected var _debug:Boolean = false;
      
      protected var var_1120:Boolean = false;
      
      protected var var_2987:String = "";
      ...
      protected var var_3050:uint = 0;
      
      protected var var_2989:Boolean = true;
      ...
      protected static function queue(param1:LibraryLoader) : void
      {
         ...
         {
            param1.var_45.load(param1.var_253,param1._context);
         }
      }
      ...
      protected static function throttle() : void
      {
         ...
         if(const_1306)
         {
            ...
            while(_loc2_ > -1)
            {
               _loc1_ = var_807[_loc2_] as LibraryLoader;
               if(_loc1_ && (_loc1_.ready || _loc1_.disposed))
               {
                  var_807.splice(_loc2_,1);
               }
               _loc2_--;
            }
            while(var_807.length < const_1782 && false)
            {
               _loc1_ = var_986.shift() as LibraryLoader;
               if(!_loc1_.ready && !_loc1_.disposed)
               {
                  var_807.push(_loc1_);
                  _loc1_.var_45.load(_loc1_.var_253,_loc1_._context);
               }
            }
         }
      }
      ...
      {
         return !!this.var_253 ? this.var_253.url : null;
      }
      ...
      {
         return !!this.var_1119 ? uint(this.var_1396 - this.var_2211) : uint(getTimer() - this.var_2211);
      }
      ...
      override public function dispose() : void
      {
         if(!disposed)
         {
            dispatchEvent(new LibraryLoaderEvent(LibraryLoaderEvent.LIBRARY_LOADER_EVENT_DISPOSE,this.var_416,this.bytesTotal,this.bytesLoaded,this.elapsedTime));
            try
            {
               this.var_45.close();
            }
      ...
      {
         this.var_253 = param1;
         this._name = parseNameFromUrl(this.var_253.url);
         this.var_1119 = false;
      ...
      {
         if(this.var_1120 && !_disposed)
         {
            this.var_1120 = false;
            if(!this.var_1119 && this.var_253)
            {
               this.var_1396 = -1;
      ...
      {
         if(!this.var_1119 && !_disposed && this.var_1397 > 0)
         {
            try
            {
               this.var_45.close();
            ...
            this.addRequestCounterToUrlRequest(this.var_253,const_1734 - this.var_1397);
            if(this.var_2989)
            {
               this.var_253.requestHeaders.push(new URLRequestHeader("pragma","no-cache"));
               this.var_253.requestHeaders.push(new URLRequestHeader("Cache-Control","no-cache"));
               this.var_2989 = false;
            }
            --this.var_1397;
            this.var_493 = 0;
            this.var_45.load(this.var_253,this._context);
            return true;
         }
         return false;
      }
      ...
      protected function loadEventHandler(param1:Event) : void
      {
         ...
         switch(param1.type)
         {
            ...
            case HTTPStatusEvent.HTTP_STATUS:
               ...
               this.debug("Load event STATUS " + this.var_416 + " for file \"" + this.url + "\"");
               break;
            case Event.UNLOAD:
               this.debug("Load event UNLOAD for file \"" + this.url + "\"");
               dispatchEvent(new LibraryLoaderEvent(LibraryLoaderEvent.LIBRARY_LOADER_EVENT_UNLOAD,this.var_416,this.bytesTotal,this.bytesLoaded,this.elapsedTime));
               break;
            case ProgressEvent.PROGRESS:
               this.debug("Load event PROGRESS for file \"" + this.url + "\"  bytes: " + this.bytesLoaded + "/" + this.bytesTotal);
               dispatchEvent(new LibraryLoaderEvent(LibraryLoaderEvent.LIBRARY_LOADER_EVENT_PROGRESS,this.var_416,this.bytesTotal,this.bytesLoaded,this.elapsedTime));
               break;
            case IOErrorEvent.IO_ERROR:
               this.debug("Load event IO ERROR for file \"" + this.url + "\"");
               if(!this.handleHttpStatus(this.var_416))
               {
            ...
            case TimerEvent.TIMER:
               ...
               if(!_disposed)
               {
                  this.var_493 = LibraryLoader.name_4;
         ...
         if(this.var_493 == LibraryLoader.const_1986)
         {
         ...
         if(this.var_493 == LibraryLoader.name_4)
         {
            if(this.prepareLibrary())
            {
               ...
               this.removeEventListeners();
               throttle();
               dispatchEvent(new LibraryLoaderEvent(LibraryLoaderEvent.LIBRARY_LOADER_EVENT_COMPLETE,this.var_416,this.bytesTotal,this.bytesLoaded,this.elapsedTime));
      ...
      protected function prepareLibrary() : Boolean
      {
         ...
         this.var_703 = this.getDefinition(this._name) as Class;
         if(this.var_703 == null)
         {
            if(!this.retry())
            {
               this.failure("Failed to find resource class \"" + this._name + "\" from library " + this.var_253.url + "!");
            }
            return false;
         }
         ...
         try
         {
            xmlClass = this.var_703.manifest as Class;
            if(xmlClass == null)
            {
               return false;
            }
      ...
      protected function debug(param1:String) : void
      {
         ...
         if(this._debug)
         {
            dispatchEvent(new LibraryLoaderEvent(LibraryLoaderEvent.LIBRARY_LOADER_EVENT_DEBUG,this.var_416,this.bytesTotal,this.bytesLoaded,this.elapsedTime));
         }
      }
      ...
      protected function failure(param1:String) : void
      {
         ...
         throttle();
         dispatchEvent(new LibraryLoaderEvent(LibraryLoaderEvent.LIBRARY_LOADER_EVENT_ERROR,this.var_416,this.bytesTotal,this.bytesLoaded,this.elapsedTime));
         dispatchEvent(new Event(LIBRARY_LOADER_FINALIZE));
      }
