package com.sulake.core.utils
{
   ...
   public class LibraryLoaderQueue extends EventDispatcher implements IDisposable
   {
      ...
      protected static const const_1782:int = 4;
      ...
      {
         return this.var_468.length + this.var_296.length;
      }
      ...
      private function next() : void
      {
         ...
         if(!disposed)
         {
            while(this.var_296.length < const_1782 && this.var_468.length > 0)
            {
               _loc1_ = this.var_468.shift();
               this.var_296.push(_loc1_);
               _loc1_.resume();
            }
         }
      }
      ...
      private function removeLoader(param1:LibraryLoader) : void
      {
         ...
         try
         {
            index = this.var_468.indexOf(loader);
            if(index > -1)
            {
               this.var_468.splice(index,1);
            }
            index = this.var_296.indexOf(loader);
            if(index > -1)
            {
               this.var_296.splice(index,1);
            }
         }
         catch(e:Error)
         {
      ...
      private function isUrlInQueue(param1:String, param2:Boolean = true) : Boolean
      {
         ...
         if(!disposed)
         {
            ...
            for each(_loc3_ in this.var_468)
            {
               if(param2)
               {
