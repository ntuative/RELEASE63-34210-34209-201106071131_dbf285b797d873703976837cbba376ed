package com.sulake.core.runtime
{
   ...
   public class ComponentContext extends Component implements IContext
   {
      ...
      public final function loadFromFile(param1:URLRequest, param2:LoaderContext) : LibraryLoader
      {
         ...
         {
            if(this.var_296[_loc3_].url == param1.url)
            {
               return this.var_296[_loc3_].loadFromFile(param1);
            }
            _loc3_++;
         }
      ...
      protected function loadErrorHandler(param1:LibraryLoaderEvent) : void
      {
         ...
         this.removeLibraryLoader(_loc2_);
         this.error("Failed to download component resource \"" + _loc2_.url + "\"!" + "\r" + _loc3_,true,Core.const_1850);
      }
         ...
         while(i < componentList.length())
         {
            ...
            if(assetCount > 0)
            {
               ...
               while(k < assetCount)
               {
                  ...
                  {
                     this.error("Manifest for component " + componentString + " contains broken asset \"" + assetName + "\"!",true,Core.const_751);
                  }
                  k++;
               }
            }
            ...
            if(componentClass == null)
            {
               this.error("Invalid component class " + componentString + "!",true,Core.const_751);
               return null;
            }
            ...
            if(component != null)
            {
               if(assetLibrary != null)
               {
                  if(component.assets != assetLibrary)
                  {
                     ...
                     this.error("Component \"" + componentString + "\" did not save provided asset library!",true,Core.const_751);
                  }
               }
      ...
      protected final function announceInterfaceAvailability(param1:IID, param2:Component) : void
      {
         ...
         while(_loc6_ < _loc5_)
         {
            ...
            if(_loc4_ == null)
            {
               this.error("Interface " + param1 + " still unavailabe!",true,Core.const_1846);
            }
      ...
      override public function dispose() : void
      {
         ...
         if(!disposed)
         {
            ...
            if(this.var_296 != null)
            {
               _loc1_ = uint(this.var_296.length);
               while(_loc1_-- > 0)
               {
                  _loc2_ = this.var_296[0];
                  this.removeLibraryLoader(_loc2_);
                  _loc2_.dispose();
               }
               this.var_296 = null;
