package com.sulake.core.window.graphics
{
   ...
   public class DrawBufferAllocator implements IDisposable
   {
      ...
      public function get allocatedByteCount() : uint
      {
         ...
         var bytes:uint = 0;
         var index:uint = this.var_710.length;
         try
         {
            while(index > 0)
            {
               index--;
               bitmap = BitmapData(this.var_710[index]);
      ...
      public function dispose() : void
      {
         ...
         {
            this._disposed = true;
            _loc1_ = uint(this.var_710.length);
            while(_loc1_ > 0)
            {
               BitmapData(this.var_710.pop()).dispose();
               _loc1_--;
            }
         }
      }
      ...
      public function allocate(param1:int, param2:int, param3:Boolean = true, param4:uint = 4.294967295E9) : BitmapData
      {
         ...
         this.var_710.push(bitmap);
         return bitmap;
      }
      
      public function free(param1:BitmapData) : void
      {
         var _loc2_:uint = this.var_710.indexOf(param1);
         if(_loc2_ < 0)
         {
