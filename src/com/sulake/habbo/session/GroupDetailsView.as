   ...
   public class GroupDetailsView
   {
      ...
      private function showGroupInfoDialog(param1:int) : void
      {
         ...
         if(this.var_176 == null)
         {
            ...
            if(_loc3_ != null)
            {
               this.var_176 = this._sessionManager.windowManager.createWindow("habbo_group_details_window","${group.window.title}",HabboWindowType.const_141,HabboWindowStyle.const_32,0 | 0,new Rectangle(10,10,300,220),null) as IWindowContainer;
               this.var_176.buildFromXML(_loc3_.content as XML) as IWindowContainer;
               _loc4_ = this.var_176.findChildByTag("close");
               if(_loc4_ != null)
               {
               ...
               {
                  _loc7_ = this.var_176.findChildByName(_loc6_);
                  if(_loc7_ != null)
                  {
         ...
         if(this.var_176 != null)
         {
            ...
            this.var_176.center();
            this.var_176.findChildByName("group_name").caption = _loc2_.name;
            this.var_176.findChildByName("group_description").caption = _loc2_.description;
            if(_loc2_.roomId < 0)
            {
               this.var_176.findChildByName("group_room_link").visible = false;
            }
            else
            {
               this.var_176.findChildByName("group_room_link").visible = true;
            }
            this._sessionManager.windowManager.registerLocalizationParameter("group.room.link","room_name",_loc2_.roomName);
            _loc8_ = this.var_176.findChildByName("group_logo") as IBitmapWrapperWindow;
            if(_loc8_ != null)
            {
      ...
      private function onGroupDetailsEvents(param1:WindowEvent) : void
      {
         ...
         var _loc3_:HabboGroupDetails = this._sessionManager.getGroupDetails(this.var_2876);
         switch(_loc2_.name)
         {
