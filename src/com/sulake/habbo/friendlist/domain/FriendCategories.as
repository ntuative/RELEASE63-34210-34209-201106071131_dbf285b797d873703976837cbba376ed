package com.sulake.habbo.friendlist.domain
{
   ...
   public class FriendCategories
   {
      ...
      private var var_474:IFriendCategoriesDeps;
      ...
      public function onFriendListUpdate(param1:IMessageEvent) : void
      {
         ...
         try
         {
            ...
            for each(updatedFriend in p.updatedFriends)
            {
               ...
               this.var_474.messenger.setFollowingAllowed(updatedFriend.id,updatedFriend.followingAllowed && updatedFriend.online);
               currentlyOnline = this.isFriendOnline(updatedFriend.id);
               if(currentlyOnline && !updatedFriend.online)
               {
                  this.var_474.messenger.setOnlineStatus(updatedFriend.id,updatedFriend.online);
                  this.var_474.notifications.addOfflineNotification(updatedFriend.name,updatedFriend.realName);
               }
               if(!currentlyOnline && updatedFriend.online)
               {
                  this.var_474.messenger.setOnlineStatus(updatedFriend.id,updatedFriend.online);
                  this.var_474.view.setNewMessageArrived();
                  this.var_474.notifications.addOnlineNotification(updatedFriend.name,updatedFriend.figure,updatedFriend.realName);
               }
      ...
      private function updateCategories(param1:Array) : void
      {
         ...
         this.findCategory(FriendCategory.const_417).received = true;
         this.findCategory(FriendCategory.const_1356).received = true;
