package com.sulake.habbo.friendlist.domain
{
   ...
   public class FriendCategory implements IDisposable
   {
      ...
      {
         var _loc4_:* = null;
         if(this.var_237.indexOf(param1) >= 0)
         {
            return;
         }
         ...
         {
            _loc4_ = this.var_237[_loc3_];
            if(_loc2_ < _loc4_.name.toLowerCase())
            {
               this.var_237.splice(_loc3_,0,param1);
               return;
            }
            _loc3_++;
         }
      ...
      public function removeFriend(param1:int) : Friend
      {
         ...
         if(_loc2_ != null)
         {
            Util.remove(this.var_237,_loc2_);
            return _loc2_;
         }
         return null;
      }
      ...
      {
         if(this.var_949 >= this.getPageCount())
         {
            this.var_949 = Math.max(0,this.getPageCount() - 1);
         }
      }
      
      public function getPageCount() : int
      {
         return Math.ceil(this.var_237.length / const_423);
      }
      
      public function getStartFriendIndex() : int
      {
         this.checkPageIndex();
         return this.var_949 * const_423;
      }
      
      public function getEndFriendIndex() : int
      {
         this.checkPageIndex();
         return Math.min((this.var_949 + 1) * const_423,this.var_237.length);
      }
