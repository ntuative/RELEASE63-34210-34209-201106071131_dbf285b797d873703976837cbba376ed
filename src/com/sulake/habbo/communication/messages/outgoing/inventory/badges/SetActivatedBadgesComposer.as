package com.sulake.habbo.communication.messages.outgoing.inventory.badges
{
   ...
   public class SetActivatedBadgesComposer implements IMessageComposer
   {
      ...
      private const const_2038:int = 5;
      ...
      {
         if(this.var_1372.length >= this.const_2038)
         {
            return;
         }
      ...
      public function getMessageArray() : Array
      {
         ...
         var _loc2_:int = 1;
         while(_loc2_ <= this.const_2038)
         {
            if(_loc2_ <= this.var_1372.length)
            {
               _loc1_.push(_loc2_);
               _loc1_.push(this.var_1372[_loc2_ - 1]);
            }
            else
            {
               _loc1_.push(_loc2_);
