package com.sulake.habbo.avatar.pets
{
   ...
   public class PetDataManager implements IPetDataManager, IDisposable
   {
      ...
      public function init() : void
      {
         ...
         for each(_loc2_ in _loc1_)
         {
            if(_loc2_ != null)
            {
               ...
               if(this.var_433.getValue(_loc4_) == null)
               {
                  this.var_433.add(_loc4_,new PetData(_loc3_));
               }
            }
         }
      }
      ...
      {
         var _loc3_:IPetData = this.var_433.getValue(param1) as IPetData;
         if(_loc3_ == null)
         {
      ...
      public function createPetFigureString(param1:int, param2:int, param3:uint) : String
      {
         ...
         var _loc4_:PetData = this.var_433.getValue(param1);
         if(_loc4_ == null)
         {
      ...
      public function populateFigureData(param1:XML) : void
      {
         ...
         speciesKeys = this.var_433.getKeys();
         for each(key in speciesKeys)
         {
            petData = this.var_433.getValue(key);
            if(petData != null)
            {
      ...
      public function assetsReady(param1:int, param2:String, param3:IPetDataListener = null) : Boolean
      {
         ...
         {
            if(this.var_1452.indexOf(_loc6_) == -1)
            {
               _loc7_ = false;
               if(this.var_618.getValue(_loc6_) == null)
               {
                  ...
                  this.var_618.add(_loc6_,_loc8_);
      ...
      private function unregisterLoadingAssets(param1:LibraryLoader) : void
      {
         ...
         for each(_loc3_ in this.var_618.getKeys())
         {
            _loc5_ = this.var_618.getValue(_loc3_);
            if(param1 == _loc5_)
            {
               _loc2_.push(_loc3_);
            }
      ...
      private function onContentLoadError(param1:Event) : void
      {
         ...
         var _loc3_:int = this.var_296.indexOf(_loc2_);
         if(_loc3_ != -1)
         {
