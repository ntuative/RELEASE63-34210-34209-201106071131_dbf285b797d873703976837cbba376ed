package com.sulake.habbo.avatar
{
   ...
   public class AvatarAssetDownloadManager extends EventDispatcher
   {
      ...
      private var var_1046:int = 3;
      
      private var var_471:AssetLoaderStruct;
      ...
      private const const_2302:int = 100;
      
      private const const_1782:int = 2;
      ...
      public function AvatarAssetDownloadManager(param1:IAssetLibrary, param2:String, param3:String, param4:AvatarStructure)
      {
         ...
         if(_loc6_ == null)
         {
            this.var_471 = this._assets.loadAssetFromFile("figuremap",_loc5_,"text/xml");
         ...
         this.var_1047 = new Timer(this.const_2302,1);
         this.var_1047.addEventListener(TimerEvent.TIMER_COMPLETE,this.onNextDownloadTimeout);
      }
      ...
      private function addMapLoaderEventListeners() : void
      {
         if(this.var_471)
         {
            this.var_471.addEventListener(AssetLoaderEvent.const_30,this.onConfigurationComplete);
            this.var_471.addEventListener(AssetLoaderEvent.const_42,this.onConfigurationError);
         }
      }
      ...
      private function removeMapLoaderEventListeners() : void
      {
         if(this.var_471)
         {
            this.var_471.removeEventListener(AssetLoaderEvent.const_30,this.onConfigurationComplete);
            this.var_471.removeEventListener(AssetLoaderEvent.const_42,this.onConfigurationError);
         }
      }
      ...
      private function onConfigurationError(param1:Event) : void
      {
         ...
         if(this.var_1046 <= 0)
         {
            HabboWebTools.logEventLog("Figuremap download error " + this.var_1323);
            Logger.log("AVATAR ASSET ERROR: Could not download figuremap.xml.");
         }
         else
         {
            Logger.log("Could not download figuremap.xml. Retries left: " + this.var_1046);
            if(this.var_1323.indexOf("?") > 0)
            {
               _loc2_ = this.var_1323 + "&retry=" + this.var_1046;
            }
            else
            {
               _loc2_ = this.var_1323 + "?retry=" + this.var_1046;
            ...
            this.var_471 = this._assets.loadAssetFromFile("figuremap",_loc3_,"text/xml");
            this.addMapLoaderEventListeners();
         }
      }
      ...
      private function generateMap(param1:XML) : void
      {
         ...
         {
            _loc3_ = new AvatarAssetDownloadLibrary(_loc2_.@id,_loc2_.@revision,this.var_2786,this._assets);
            _loc3_.addEventListener(Event.COMPLETE,this.libraryComplete);
            for each(_loc4_ in _loc2_.part)
            {
               _loc5_ = _loc4_.@type + ":" + _loc4_.@id;
               _loc6_ = this.var_1324[_loc5_];
               if(_loc6_ == null)
               {
                  _loc6_ = [];
               }
               _loc6_.push(_loc3_);
               this.var_1324[_loc5_] = _loc6_;
            }
         }
      }
      ...
      public function loadFigureSetData(param1:IAvatarFigureContainer, param2:IAvatarImageListener) : void
      {
         ...
         if(!this.var_2037)
         {
            this.var_1325.push([param1,param2]);
            return;
         }
      ...
      private function libraryComplete(param1:Event) : void
      {
         ...
         {
            delete this.var_1045[_loc3_];
         }
         while(_loc6_ < this.var_858.length)
         {
            _loc5_ = this.var_858[_loc6_];
            if(_loc5_.libraryName == (param1.target as AvatarAssetDownloadLibrary).libraryName)
            {
               this.var_858.splice(_loc6_,1);
            }
            _loc6_++;
         }
         this.var_1047.start();
      ...
      {
         var _loc1_:* = null;
         while(this.var_857.length > 0 && this.var_858.length < this.const_1782)
         {
            _loc1_ = this.var_857[0];
            _loc1_.startDownloading();
            this.var_858.push(this.var_857.shift());
         }
      }
      
      private function addToQueue(param1:AvatarAssetDownloadLibrary) : void
      {
         if(!param1.isReady && this.var_857.indexOf(param1) == -1 && this.var_858.indexOf(param1) == -1)
         {
