package com.sulake.habbo.avatar.geometry
{
   public class GeometryItem extends Node3D
   {
      ...
      public function GeometryItem(param1:XML, param2:Boolean = false)
      {
         ...
         this.var_1620 = parseFloat(param1.@radius);
         this._normal = new Vector3D(parseFloat(param1.@nx),parseFloat(param1.@ny),parseFloat(param1.@nz));
         this.var_2523 = parseInt(param1.@double) as Boolean;
         this.var_2524 = param2;
      }
      
      public function getDistance(param1:Vector3D) : Number
      {
         var _loc2_:Number = Math.abs(param1.z - this.transformedLocation.z - this.var_1620);
         var _loc3_:Number = Math.abs(param1.z - this.transformedLocation.z + this.var_1620);
