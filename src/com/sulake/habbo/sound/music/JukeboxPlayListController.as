package com.sulake.habbo.sound.music
{
   ...
   public class JukeboxPlayListController implements IPlayListController
   {
      ...
      public function JukeboxPlayListController(param1:HabboSoundManagerFlash10, param2:HabboMusicController, param3:IEventDispatcher, param4:IConnection)
      {
         ...
         this.var_752 = [];
         this.var_752.push(new NowPlayingMessageEvent(this.onNowPlayingMessage));
         this.var_752.push(new JukeboxSongDisksMessageEvent(this.onJukeboxSongDisksMessage));
         this.var_752.push(new JukeboxPlayListFullMessageEvent(this.onJukeboxPlayListFullMessage));
      ...
      {
         this.var_231.stop(this.priority);
         this.var_455 = -1;
         this.var_1862 = -1;
         this.var_662 = false;
      }
      ...
      private function onNowPlayingMessage(param1:IMessageEvent) : void
      {
         ...
         this.var_662 = _loc3_.currentSongId != -1;
         if(_loc3_.currentSongId >= 0)
         {
            this.var_231.playSong(_loc3_.currentSongId,HabboMusicPrioritiesEnum.const_136,_loc3_.syncCount / 1000,0,1,1);
            this.var_455 = _loc3_.currentSongId;
         }
         else
         {
            this.stopPlaying();
         ...
         {
            this.var_231.addSongInfoRequest(_loc3_.nextSongId);
         }
         this.var_1862 = _loc3_.currentPosition;
         this._soundManager.events.dispatchEvent(new NowPlayingEvent(NowPlayingEvent.const_103,HabboMusicPrioritiesEnum.const_136,_loc3_.currentSongId,_loc3_.currentPosition));
      }
      ...
      private function onJukeboxSongDisksMessage(param1:IMessageEvent) : void
      {
         ...
         while(_loc4_ < _loc3_.songDisks.length)
         {
            ...
            _loc7_ = this.var_231.getSongInfo(_loc5_) as SongDataEntry;
            if(_loc7_ == null)
            {
               _loc7_ = new SongDataEntry(_loc5_,-1,null,null,null);
               if(this.var_799.indexOf(_loc5_) < 0)
               {
                  this.var_799.push(_loc5_);
                  this.var_231.requestSongInfoWithoutSamples(_loc5_);
               }
            }
            _loc7_.diskId = _loc6_;
            this.var_530.push(_loc7_);
            _loc4_++;
         }
      ...
      private function onSongInfoReceivedEvent(param1:SongInfoReceivedEvent) : void
      {
         ...
         {
            _loc4_ = this.var_530[_loc2_];
            if(_loc4_.id == param1.id)
            {
               _loc5_ = _loc4_.diskId;
               _loc6_ = this.var_231.getSongInfo(param1.id) as SongDataEntry;
               if(_loc6_ != null)
               {
                  _loc6_.diskId = _loc5_;
                  this.var_530[_loc2_] = _loc6_;
               }
               break;
            }
            _loc2_++;
         }
         var _loc3_:int = this.var_799.indexOf(param1.id);
         if(_loc3_ >= 0)
         {
            this.var_799.splice(_loc3_,1);
         }
         if(this.var_799.length == 0)
         {
            this._events.dispatchEvent(new PlayListStatusEvent(PlayListStatusEvent.const_106));
