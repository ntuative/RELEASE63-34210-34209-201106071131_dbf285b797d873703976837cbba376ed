package com.sulake.habbo.catalog.marketplace
{
   ...
   public class MarketplaceConfirmationDialog implements IGetImageListener
   {
      ...
      private var var_778:MarketPlaceLogic;
      ...
      private var var_101:MarketPlaceOfferData;
      ...
      public function showConfirmation(param1:int, param2:MarketPlaceOfferData) : void
      {
         ...
         this.var_101 = param2;
         if(!this.var_778 || !this._catalog || !this._catalog.localization)
         {
            return;
         }
         if(this._window)
         ...
         if(_loc3_)
         {
            if(param1 == this.var_778.const_2227)
            {
               _loc3_.text = "${catalog.marketplace.confirm_header}";
            }
            if(param1 == this.var_778.const_2071)
            {
         ...
         if(_loc3_)
         {
            _loc3_.text = "${" + this.var_778.getNameLocalizationKey(param2) + "}";
         }
         ...
         if(_loc3_)
         {
            ...
            _loc4_ = _loc4_.replace("%price%",this.var_101.price);
            _loc3_.text = _loc4_;
         }
         ...
         if(_loc3_)
         {
            ...
            {
               _loc4_ = _loc5_.raw;
               _loc4_ = _loc4_.replace("%days%",this.var_778.averagePricePeriod.toString());
               _loc6_ = this.var_101.averagePrice == 0 ? " - " : this.var_101.averagePrice.toString();
         ...
         if(_loc3_)
         {
            ...
            {
               _loc4_ = _loc5_.raw;
               _loc4_ = _loc4_.replace("%count%",this.var_101.offerCount);
               _loc3_.text = _loc4_;
            }
            else
            {
      ...
      private function setImage() : void
      {
         var _loc1_:* = null;
         var _loc2_:* = null;
         if(!this.var_101 || !this._window || !this._roomEngine)
         {
            return;
         }
         if(!this.var_101.image)
         {
            if(this.var_101.furniType == 1)
            {
               _loc1_ = this._roomEngine.getFurnitureIcon(this.var_101.furniId,this);
            }
            else if(this.var_101.furniType == 2)
            {
               _loc1_ = this._roomEngine.getWallItemIcon(this.var_101.furniId,this);
            }
            if(_loc1_ && _loc1_.data)
            {
               this.var_101.image = _loc1_.data as BitmapData;
               this.var_101.imageCallback = _loc1_.id;
            }
         }
         if(this.var_101.image != null)
         {
            ...
            if(_loc2_)
            {
               ...
               _loc2_.bitmap.draw(this.var_101.image,new Matrix(1,0,0,1,(_loc2_.width - this.var_101.image.width) / 2,(_loc2_.height - this.var_101.image.height) / 2));
            }
         }
      }
      ...
      private function eventHandler(param1:WindowEvent, param2:IWindow) : void
      {
         ...
         {
            case "buy_button":
               this._catalog.buyMarketPlaceOffer(this.var_101.offerId);
               this.hide();
               break;
      ...
      {
         if(this.var_101 && this.var_101.imageCallback == param1)
         {
            this.var_101.image = param2;
            this.setImage();
         }
      }
