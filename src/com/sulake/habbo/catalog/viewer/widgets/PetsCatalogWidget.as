package com.sulake.habbo.catalog.viewer.widgets
{
   ...
   public class PetsCatalogWidget extends CatalogWidget implements ICatalogWidget, IPetDataListener, IPetImageListener
   {
      ...
      override public function init() : Boolean
      {
         ...
         _loc2_.addEventListener(WindowEvent.const_117,this.onTextWindowEvent);
         ...
         this.var_503 = this.getPetTypeIndexFromProduct(_loc3_.localizationId);
         if(this.var_503 >= 8)
         {
            return false;
         }
         this.updateAvailableBreeds(_loc3_.localizationId);
         this.var_2222 = _loc3_.localizationId;
         this.var_360 = [];
         if(page && false && page.viewer.catalog)
         {
            ...
            {
               _loc5_ = _loc4_.avatarRenderManager.petDataManager.getPetData(this.var_503,this);
               if(_loc5_)
               {
         ...
         this._offers.add(this.var_503,_loc3_);
         events.addEventListener(WidgetEvent.CWE_SELECT_PRODUCT,this.onSelectProduct);
         events.addEventListener(WidgetEvent.CWE_COLOUR_INDEX,this.onColourIndex);
         events.addEventListener(WidgetEvent.const_975,this.onApproveNameResult);
      ...
      private function onColourIndex(param1:CatalogWidgetColourIndexEvent) : void
      {
         ...
         if(this.var_521 < 0 || this.var_521 > this.var_360.length)
         {
            this.var_521 = 0;
         }
         this.updateImage();
      ...
      {
         if(param1.productCode == this.var_2222)
         {
      ...
      private function parseSellableBreeds(param1:Array) : Array
      {
         ...
         {
            if(_loc3_.type == this.var_503 && _loc3_.sellable)
            {
      ...
      private function updateBreedSelections() : void
      {
         ...
         while(_loc3_ < this.var_210.length)
         {
            ...
            _loc2_.push(_loc1_.getKey(this.getRaceLocalizationKey(this.var_503,_loc5_),this.getRaceLocalizationKey(this.var_503,_loc5_)));
            _loc3_++;
         }
      ...
      private function updateImage() : void
      {
         ...
         if(this.var_210 == null || this.var_779 >= this.var_210.length)
         {
            return;
         }
         var _loc2_:* = 0;
         if(this.var_521 >= 0 && this.var_521 < this.var_360.length)
         {
            _loc7_ = this.var_360[this.var_521] as IPetColor;
            if(_loc7_ != null)
            {
               _loc2_ = uint(_loc7_.rgb);
            }
         }
         var _loc3_:int = this.var_210[this.var_779] as int;
         var _loc4_:IAvatarImage = (page.viewer.catalog as HabboCatalog).avatarRenderManager.createPetImage(this.var_503,_loc3_,_loc2_,AvatarScaleType.const_60,this);
         if(_loc4_ == null)
         {
            return;
         }
         _loc4_.initActionAppends();
         _loc4_.appendAction(AvatarAction.const_321,AvatarAction.const_828);
      ...
      private function getPurchaseParameters() : String
      {
         ...
         if(this.var_521 >= this.var_360.length)
         {
            return "";
         }
         var petColor:IPetColor = this.var_360[this.var_521] as IPetColor;
         var color:uint = petColor.rgb;
         var breed:int = this.var_210[this.var_779] as int;
