package com.sulake.habbo.ui.widget.roomqueue
{
   ...
   public class RoomQueueWidget extends RoomWidgetBase
   {
      ...
      override public function registerUpdateEvents(param1:IEventDispatcher) : void
      {
         ...
         param1.addEventListener(RoomWidgetRoomQueueUpdateEvent.const_650,this.onQueueStatus);
      ...
      override public function unregisterUpdateEvents(param1:IEventDispatcher) : void
      {
         ...
         param1.removeEventListener(RoomWidgetRoomQueueUpdateEvent.const_650,this.onQueueStatus);
      }
      ...
      private function onQueueStatus(param1:RoomWidgetRoomQueueUpdateEvent) : void
      {
         ...
         if(param1.isActive)
         {
            this.var_1752 = param1.type;
            this.var_1444 = false;
            this.var_1167 = param1.position;
         }
         else
         {
            this.var_1444 = true;
         }
         this.var_2360 = param1.hasHabboClub;
         this.var_1751 = param1.isClubQueue;
         localizations.registerParameter("room.queue.position","position",this.var_1167.toString());
         localizations.registerParameter("room.queue.position.hc","position",this.var_1167.toString());
         localizations.registerParameter("room.queue.spectator.position","position",this.var_1167.toString());
         localizations.registerParameter("room.queue.spectator.position.hc","position",this.var_1167.toString());
         this.showInterface();
      }
         ...
         if(_loc1_ != null && _loc2_ != null && _loc3_ != null)
         {
            switch(this.var_1752)
            {
               case RoomWidgetRoomQueueUpdateEvent.const_446:
                  _loc1_.caption = !!this.var_1751 ? "${room.queue.position.hc}" : "${room.queue.position}";
                  _loc2_.caption = "${room.queue.spectatormode}";
                  _loc3_.visible = this.var_1444;
                  break;
               case RoomWidgetRoomQueueUpdateEvent.const_650:
                  _loc1_.caption = !!this.var_1751 ? "${room.queue.spectator.position.hc}" : "${room.queue.spectator.position}";
      ...
      private function exitQueue(param1:WindowMouseEvent) : void
      {
         ...
         var _loc2_:RoomWidgetRoomQueueMessage = new RoomWidgetRoomQueueMessage(RoomWidgetRoomQueueMessage.const_772);
      ...
      {
         messageListener.processWidgetMessage(new RoomWidgetRoomQueueMessage(RoomWidgetRoomQueueMessage.const_899));
      }
      ...
      private function changeQueue(param1:WindowMouseEvent) : void
      {
         ...
         if(this.var_1752 == RoomWidgetRoomQueueUpdateEvent.const_446)
         {
            _loc2_ = new RoomWidgetRoomQueueMessage(RoomWidgetRoomQueueMessage.CHANGE_TO_SPECTATOR_QUEUE);
         }
         else
         {
            _loc2_ = new RoomWidgetRoomQueueMessage(RoomWidgetRoomQueueMessage.const_887);
         }
         messageListener.processWidgetMessage(_loc2_);
         this.removeWindow();
      }
